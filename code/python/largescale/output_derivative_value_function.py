	...
	for t in range(1, tc):
		...
		for b in range(1, bc + 1):
			for i in range(0, action[b] + 1):
				t_value_function[b] += market_price_distribution[i] * (
				ctr + value_function[b - i] - value_function[b])
...
if __name__ == "__main__":
	...
	with open(DataPath.rtb_mdp_path + campaign + "/derivative_value_function.txt", "w") as fout:
